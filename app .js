...
//  Rules: MAX over limit only. Any player can bowl.
//  LocalStorage: 24h persist.
// ═══════════════════════════════════════════════
...
let G = {
  screen: 'setup',
  setup: { teamA:'', teamB:'', overs:20, players:11, batNames:[], bowlNames:[], tiers:[] },
  match: null,
  inn1: null,
};
let tierRows = []; // working tier list during setup
let pickedDis = null;
...
function buildPlayerGrids(count) {
  ['batGrid','bowlGrid'].forEach((id, side) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      inp.type = 'text';
      inp.id = `pn_${side}_${i}`;
      inp.placeholder = side === 0 ? `Batsman ${i+1}` : `Bowler ${i+1}`;
      inp.maxLength = 18;
      inp.value = side===0 ? (G.setup.batNames[i]||'') : (G.setup.bowlNames[i]||'');
...
function toggleRest(i, isRest) {
  ...
  if (isRest) tierRows = tierRows.slice(0, i+1); // remove tiers after this
  renderTiers();
  checkFeasibility();
}

function addTier() {
  // if last tier is rest, convert it first
...
// ── Get max overs for bowler by rank (0-based, order of first bowl) ──
...
function startMatch() {
  ...
  const batNames=[], bowlNames=[];
  for(let i=0;i<pl;i++){
    batNames.push($(`pn_0_${i}`)?.value.trim() || `Batsman ${i+1}`);
    bowlNames.push($(`pn_1_${i}`)?.value.trim() || `Bowler ${i+1}`);
  }
  ...
  G.setup = { teamA, teamB, overs:ov, players:pl, batNames, bowlNames, tiers: tierRows.map(t=>({...t})) };
  G.inn1 = null;
...
function initMatch(innings) {
  ...
  G.match = {
    ...
    // bowling
    bowlNames,          // all eligible bowlers (by name)
    bowlOrder:[],       // names in order they first bowled
    bowlMap:{},         // name -> {balls,runs,wickets,wides,noballs,rank,maxOv}
    curBowler:null,
    prevBowler:null,
    needBowler:true,
    history:[],
...
function ball(runs, extra) {
  ...
  // batsman credit (not bye/legbye/wide)
...
function swapBat() {
  ...
  m.striker=m.nonStriker; m.nonStriker=t;
}
...
function saveSnap() {
  const snap = JSON.stringify({
    ...
    doneOvers:G.match.doneOvers.length,
    ...
    needBowler:G.match.needBowler, done:G.match.done,
  });
...
function openBowlerModal() {
  ...
  const list=$('bowlerList'); list.innerHTML='';
...
function renderAll() { renderScore(); renderBalls(); renderBatsmen(); renderBowlFigs(); }
...
function renderBalls() {
  ...
  for(let i=legal;i<6;i++){
    const d=el('div','ball bgh'); d.textContent='·'; row.appendChild(d);
  }
  const overNum=Math.floor(m.balls/6)+1;
...
function renderScorecard() {
  ...
  const bwp=$('bowlSC'); bwp.innerHTML='';
  m.bowlOrder.forEach(name=>{
    const b=m.bowlMap[name];
    if(!b) return;
    const ov=Math.floor(b.balls/6), rb=b.balls%6;
...
function doReset() {
  ...
  clearState();
  tierRows=[];
  G={screen:'setup',setup:{teamA:'',teamB:'',overs:20,players:11,batNames:[],bowlNames:[],tiers:[]},match:null,inn1:null};
  $('inn2Btn').style.display='none';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Wire input change listeners (elements now exist)
